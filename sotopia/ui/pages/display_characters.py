...
from sotopia.ui.socialstream.utils import initialize_session_state
...
def rendering_character(character : AgentProfile) -> None:
    ...
    personal_info = ""
    personal_info += display_field("Age", character.age)
    ...
    with st.expander("Additional Information", expanded=False):
        ...
        additional_info += display_field("Moral Values", ', '.join(character.moral_values))
        additional_info += display_field("Schwartz Personal Values", ', '.join(character.schwartz_personal_values))
        additional_info += display_field("Personality and Values", character.personality_and_values)
        additional_info += display_field("Decision Making Style", character.decision_making_style)
